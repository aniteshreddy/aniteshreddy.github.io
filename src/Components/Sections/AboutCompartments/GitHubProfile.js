...
function GitHubRepositories() {
    ...
    async function fetchRepositories() {
      try {
        const response = await axios.get(
          "https://api.github.com/users/aniteshreddy/repos?sort=updated&direction=desc"
        );
  ...
  const loadMoreRepositories = () => {
    setVisibleRepositories(
      (prevVisibleRepositories) => prevVisibleRepositories + 3
    );
  };
    ...
    <div className="githubData">
      {repositories.length > 0 ? (
        <div className="border-end">
          <h3 className="border-bottom  small p-3  m-0">_my_repository</h3>
